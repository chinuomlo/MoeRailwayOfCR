...
public class sidc1 implements BlockEntityRenderer<sidc1Entity> {
   private final TextRenderer textRenderer;
 
   public sidc1(BlockEntityRendererFactory.Context ctx) {
      this.textRenderer = ctx.getTextRenderer();
      Logger LOGGER = LoggerFactory.getLogger("modid");
      LOGGER.info("进入BER");
   }
   private static int getColor(sidc1Entity sign) {
      int i = 1;
      int r = 255;
      int g = 255;
      int b = 255;
   ...
   @Override
   public void render(sidc1Entity signBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
   ...
   public void Rerender(sidc1Entity signBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
      BlockState blockState = signBlockEntity.getCachedState();
      matrixStack.push();
      signBlockEntity.toUpdatePacket();
      NbtCompound nbt = signBlockEntity.toInitialChunkDataNbt();
